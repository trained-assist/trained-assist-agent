...
const HH_DIGEST_TASK = `SCHEDULED: обработай новые отклики HH. Выполни автоматически без лишних вопросов.

1. context_get('hh', 'active_vacancy') → если found=false → отправь «⏸ HH: нет активной вакансии. Выбери вакансию в следующей сессии.» и завершай.
2. hh_get_negotiations для этой вакансии со статусом response, limit=100.
3. context_get('hh', 'ats_config') → возьми сохранённый ATS конфиг. Если нет — используй базовые критерии.
4. Оцени каждого нового кандидата → score 0-10 + вердикт.
5. Отправь дайджест:
   📊 HH | {название вакансии}
   Новых: N | Целевых (≥7): M | Отказов: K
   [если M>0 — топ-3 целевых: имя, текущая должность, score]`.trim();
