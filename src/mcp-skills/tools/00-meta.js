...
const SKILLS = [
  ...
  {
    id: 'efi-qr',
    name: 'EFI QR — быстрые QR-коды Школы Ефимовой',
    description: 'Генерирует QR-коды для мерчантов Школы Ефимовой: счёт на оплату (покупатель вводит свой ИНН и скачивает инвойс PDF), форма сбора контактов, редирект-QR. Активный мерчант выбирается один раз — все банковские реквизиты подставляются автоматически.',
    requires: 'EFI_QR_URL и EFI_QR_TOKEN в env на VM. Затем efi_set_merchant(id) для выбора города/франчайзи.',
  },
  {
    id: 'api-from-website',
