...
async function driveReq(method, apiPath, body, sa) {
  // Implementation of driveReq function
}
  const token = await getAccessToken(sa);
...
const proj = loadProject(project_id);
        ...
        const completeness = calcCompleteness(proj.signals);
        const answeredCount = Object.values(proj.signals)
          .filter(v => v !== null && v !== undefined && typeof v === 'boolean').length;
        const openQs = getOpenQuestions(proj.signals);

        await writeTab(proj.sheet_id, '📊 Риски', buildRisksRows(proj, assessment));
        await writeTab(proj.sheet_id, '📅 План проекта', buildPlanRows(proj));
        await writeTab(proj.sheet_id, '💬 Q&A с клиентом', buildQARows(proj));
        await writeTab(proj.sheet_id, '👥 Роли и ресурсы', buildRolesRows(proj));
        ...
        return {
          ...
          info_chunks: proj.info_chunks.length,
