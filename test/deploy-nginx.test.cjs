...
function fixture(t) {
  ...
  for (const [name,body] of Object.entries({sudo:'exec "$@"',nginx:'echo test >> "$CALLS"; ! grep -q BAD "$NGINX_ROOT/sites-enabled/relay" 2>/dev/null',systemctl:'echo "$*" >> "$CALLS"; test "${FAIL_RELOAD:-}" != 1 || test "$1" != reload'})) {
