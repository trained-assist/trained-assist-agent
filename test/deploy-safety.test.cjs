...
function fixture(t, failure) {
 ...
 const repo=path.join(root,'repo'),bin=path.join(root,'bin');fs.mkdirSync(bin);fs.mkdirSync(path.join(repo,'node_modules'),{recursive:true});fs.mkdirSync(path.join(repo,'systemd'));
