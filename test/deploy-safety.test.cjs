...
function fixture(t, failure, v2 = false) {
 ...
 if(v2){fs.mkdirSync(path.join(root,'data'));fs.writeFileSync(path.join(root,'data','execution-authority.json'),'{}');}
 mock('git',`case \"$*\" in *restart-execution.js*) ${v2?'exit 1':'exit 0'};; esac\n`+'case "$*" in *rev-parse*) echo oldcommit;; esac\nexit 0');
 ...
 return {result,repo,log:fs.readFileSync(log,'utf8')};
}
...
test('rollback never starts legacy auto-resumer over v2 authority',t=>{
 const f=fixture(t,'readiness',true);assert.equal(f.result.status,43,f.result.stderr+f.result.stdout);
 assert.match(f.result.stdout,/Rollback refused/);assert.doesNotMatch(f.log,/reset --hard oldcommit/);
 assert.equal(f.log.split('\n').filter(x=>x.includes('restart-coordinator.py --ready')).length,1);
});
