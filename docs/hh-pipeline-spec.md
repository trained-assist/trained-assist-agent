...
| 1. Синк переписки | ⚠️ | `syncHhMessagesToHistory()` добавлен PR #567, dedup по `hh_id`. **Нет:** вызова в фоновом loop (только при /hh/review); инкрементального синка по `updated_at`; пагинации /messages (кэп 50) |
| 2. Пересчёт после синка | ⚠️ | `scoreUnscoredCandidates()` re-скорит если `lastCandMsg.timestamp > scored_at`. **Проблема:** фоновый loop не видит новых сообщений — они приходят только при открытии /hh/review |
| 3. Score (matched/gaps) | ✅ | `evaluateCandidate()` hh-scoring.js, matched/gaps/verdict/score/reasoning |
| 4. Генерация сообщения | ⚠️ | `generateDraftMessages()` вызывается после scoring в фоне. **Нет:** matched/gaps из ats_result в промпте генерации |
| 5. Bullshit Guard | ❌ | Полностью отсутствует. Сообщения отправляются без проверки |
| 6. Список кандидатов | ⚠️ | Табы "Все" + "Неотвеченные" (PR #565). **Нет:** "Отвеченные", "Написали—молчат", "Ещё не писали"; нет превью последнего сообщения |
| 7. Профиль кандидата | ❌ | Нет отдельной страницы `/hh/candidate/{id}`. Только inline карточки |
| 8. Лог синка | ⚠️ | `/hh/sync-log` есть (scoring runs). **Нет:** кол-во новых сообщений, сгенерированных, Guard статистика, ошибки API |
