...
| 1. Синк переписки | ✅ | `syncHhMessagesToHistory()` PR #567+#568. Инкрементальный синк по `updated_at` + `last_hh_message_at`, пагинация /messages, вызывается в фоновом loop (incremental=true) и при page-load (cap=15) |
| 2. Пересчёт после синка | ✅ | Фоновый loop (`runHhScoringForUser`) вызывает синк сообщений перед `scoreUnscoredCandidates` — видит новые ответы кандидатов и пересчитывает PR #568 |
...
| 6. Список кандидатов | ✅ | 5 табов: Неотвеченные / Молчат / Ещё не писали / Диалог / Все (PR #565, #569). Нет превью последнего сообщения в карточке |
| 7. Профиль кандидата | ❌ | Нет отдельной страницы `/hh/candidate/{id}`. Только inline карточки |
| 8. Лог синка | ⚠️ | `/hh/sync-log` есть + "Новых сообщ." + "С активностью" (PR #568). **Нет:** Guard статистика, ошибки API |
